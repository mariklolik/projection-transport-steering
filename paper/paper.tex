...
$+0.20$ for prompting. In Section~\ref{sec:results} we give every method an
equal tuning budget and find that they keep that selectivity when the
questions change, where a faithful reimplementation of MiMiC keeps half of its
own, and that they retain more \crok{} than any baseline that matches them. In
Section~\ref{ssec:ninetb} they replicate at $4.5\times$ scale and transfer to
ARC-Challenge and GSM8K with no refitting, and every operator is $O(d)$ per token
...
instruction-following, the concrete gap this family has to close.
...
and we derive locality, dose control and serving cost from one optimality
statement rather than fixing them by hand.\looseness=-1
...
Nguyen et al.~\cite{nguyen2026alqr} prove LQR-optimality of a control input. We prove a different statement in Theorem~\ref{thm:ot}, namely the optimal map
...
Cor.~\ref{cor:additive}. We leave that baseline ungated, so its design leaves the
...
with each action (Appendix~\ref{app:fulltables}). Zhang et al.~\cite{zhang2026wired} report the dose--response our per-sample
dose exploits.
...
with equal projection laws is then identically zero.
\end{corollary}
...
We can then design gate and action separately without losing optimality.
...
We state two further properties of the operator: that it does not depend
...
sound, and we search $t^\star$ and the threshold quantile on the tuning
split like every other hyperparameter. We regenerate nothing. In both regimes the
...
included, we give the same tuning budget of eight configurations, searched on
...
The \emph{surgical metrics} ask whether the edit hit only the behavior it
was aimed at. Pairing
...
confidence.} We first test the geometric premise that selective steering relies on. In
...
Thm.~\ref{thm:gate}b states that this is not a correlation to be
...
\textbf{Main result.} To measure how much of the behavior an operator can
remove for what it damages, we compare across five MMLU subsets. We find that
gated ablation attains
...
\end{tabular}}
\caption{\textbf{Equal-budget head-to-head over five resampled MMLU
subsets.} Our gated local action holds $+0.206\pm0.059$ selectivity at
retention $0.713$, the smallest spread in the table, and the
score-proportional dose reaches $+0.231$; the tuned additive dose reaches
$+0.256$ by retaining $0.621$. Every method runs the
...
\textbf{What survives a change of questions.} To separate what a method has
learned about the behavior from what it has learned about one split, we carry
each winner unchanged to five resampled question sets. In that setting the tuning-split ordering does not hold. MiMiC keeps
...
fires on most flagged traces and full ablation on the marginal ones costs
\crok{}. So we let the strength of the action follow the gate score
instead of switching on it, the interpolation $g_\lambda$ the method already
defines. Under the budget it reaches $+0.267$, within noise of the tuned
additive dose at $+0.271$ and above MiMiC at $+0.263$, and it does not win in
the way the diagnosis suggested. The
...
To see \emph{why} gate and local action compose in the measured movement of
...
\textbf{Detection remains the main bottleneck.} We find the LDA gate separates \ocw{} from \crok{} at trace-level AUROC
...
removal at the same rate in retention (Appendix~\ref{app:ablation}), the
practical form of that constraint. Because the constraint is detection, we ask where in the network detection
is best. We refit
...
generation it was never meant to touch, so to test that we measure three tasks the
directions never saw, reported in Appendix~\ref{app:capability}. We find that the dose
that removes
...
\textbf{Cross-domain transfer.} To test whether an operator fitted for one
domain is safe in another, we apply the MMLU-fitted directions, maps and
...
LDA gate errs the other way and fires on nothing, the direction we want an
unrefitted threshold to fail in. Label-free re-anchoring of $\tau$ to
...
to the identity where the behavior is absent, as the framework asks of it.\looseness=-1
...
\end{tabular}}
\caption{\textbf{Cross-domain transfer with no refitting.} On GSM8K-MCQ,
where the fitted tail is absent, every conditional operator of ours is the
exact identity while the tuned additive dose removes all eleven \crok{} and
the tuned CAST gate costs $6.3$ accuracy points. Every method keeps the
configuration it won on the MMLU tuning split, and we apply it to
...
on the whole 300-question workload on one idle H100, the sequential gate takes
$45.5$ seconds against $85.8$ for the post-hoc one and $43.5$ unsteered, or
$1.05\times$ against $1.97\times$, because the post-hoc gate must score every
finished trace and regenerate the $223$ it flags.
...
(Appendix~\ref{app:serving}). What it separates instead is the number of
passes. Strength interpolates as
...
Because decoding is deterministic, our evaluation seeds resample the
questions rather than the training runs. Architecture transfer is confirmed on OLMo-2/TruthfulQA, while other
...
In this work we turned an empirical failure of activation steering into a
set of theorems and a method. The
...
gated operators sit at the unsteered value on all four measurements. The
