# Perplexity of the manuscript's prose against the reference corpus, under one
# fixed scorer, so the comparison is a distance rather than an absolute.
#   python -m paper.ppl_check /path/to/prose_dir
...
def sent_ppl(model, tok, text: str) -> float:
    """One forward over the whole sentence, so short lines are scored too."""
    ids = tok(text, return_tensors="pt").input_ids.to(model.device)
    if ids.shape[1] < 4:
        return float("nan")
