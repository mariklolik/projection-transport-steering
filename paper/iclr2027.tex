...
holds the single-pass PTS decisions, and the last two rows keep the PTS decision with no
edit and with a random direction at the PTS dose (mean of 100). $^*$: $95\%$
...
least as selective as the action (Appendix~\ref{app:refs}). The evidence
covers two behaviors in three model families of 2 to 7 billion parameters with
one toxicity classifier, and the multiple-choice protocols became public only
together with their results, so their precedence rests on local commit times.
