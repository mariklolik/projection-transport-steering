...
editing its residual stream, but most edits in use act on every input, so an
edit strong enough to remove a behavior also changes the inputs that never
showed it. We propose Projection-Transport Steering (PTS), a framework that
separates when to intervene from how. The action is the optimal-transport map
of a low-dimensional behavioral projection onto a target law, which we show is
the unique minimum-displacement edit that leaves the rest of the residual
stream untouched. The decision is a likelihood-ratio test on a learned
detection statistic. We show that the selectivity of a conditional edit
decomposes exactly into the decision's error rates and the action's conversion
rates, which predicts gated selectivity from one ungated run and bounds it from
unsteered answers alone. On overconfident multiple-choice reasoning, PTS is the
most selective of eight methods on held-out questions and lowers calibration
error at non-inferior accuracy. On free-text toxicity in three model families
it is more selective than refusal on the same decision and than random
directions, above a global shift and prompting in two of them, and its quantile
transport map outperforms a shift of equal displacement.
\end{abstract}
...
However, these methods apply the same edit to every input, so an edit strong
enough to remove a behavior also acts on the inputs that never showed it: a
refusal vector at a useful strength raises refusal on harmful and harmless
prompts alike~\citep{arditi2024refusal}.

The difficulty is sharpest for a behavior that a model shows only on some
inputs, such as overconfidence in multiple-choice reasoning. The answers to
change and the answers to protect are both confident, one wrong and one right.
On Gemma-2-2B, the global shift that eliminates every overconfident-wrong
...
a few directions read off the model's own labeled outputs. The
\emph{decision} is a likelihood-ratio test on a detection statistic fitted to
tell the target state from the protected one, and the action runs only where
the test fires. Table~\ref{tab:family} places published operators in this
frame.
...
for particular targets, and the monotone quantile map covers the rest. The
decision is accounted for exactly. The selectivity of a conditional edit, the
share of target outputs it removes minus the share of protected outputs it
loses, equals the decision's true-positive rate times the rate at which the
action converts flagged targets, minus its false-positive rate times the rate
at which it converts flagged protected outputs
(Proposition~\ref{prop:account}). The identity predicts gated selectivity from
one ungated run, gives the ceiling of a decision from unsteered answers
...
with correlation $0.97$ (Sec.~\ref{ssec:law}).
...
on 3{,}000 MMLU questions, lowers the share of overconfident-wrong answers by
15 points and the calibration error from $0.354$ to $0.255$, and its direction
outperforms a hundred random directions through the same decision
(Sec.~\ref{sec:results}).
\item Free-text control on toxic continuation in three model families, where
PTS is more selective than refusal on its own decision in all three and than
the global shift and prompting in two, and where the quantile transport map,
selected by the tuning rule, outperforms a shift of equal mean displacement; and a head-to-head in the
setting of PCHI, where PTS is more selective than the learned head rescaling
under the same probe (Sec.~\ref{sec:transfer}).
\end{itemize}
...
\hline
\end{tabular}}
\caption{\textbf{Steering operators as a transfer function and a decision.}
Each row applies a transfer function $g$ to a $k$-dimensional projection $p$
of the residual stream. PTS derives $g$ from a target law $\nu$
(Theorem~\ref{thm:ot}) and accounts for its decision exactly
...
\end{tikzpicture}
\caption{\textbf{PTS reaches removal--retention points that a global edit
cannot.} Held-out questions (a) and prompts (b), Gemma-2-2B; up and to the
right is better, and dotted lines mark equal selectivity. Each marker is one
method at its tuned configuration; the orange curve is one shift at increasing
dose, and gray points send random directions through the PTS decision.}
\label{fig:frontier}
\end{figure}
...
positions, consistent with the confidence coordinate that the PTS action
moves. Activations predict whether an
...
Theorem~\ref{thm:ot} turns the choice of edit
into a choice of target law, made on a tuning split (Sec.~\ref{sec:exp}). Read backwards, it says when the shift is
optimal.
...
case in which conversion depends on $s$. The decision is
...
Gemma-2-2B-it~\citep{gemma2024}. The multiple-choice comparison is repeated on
Qwen2.5-7B-Instruct, a second model family on which everything is refitted, and
on ARC-Challenge~\citep{clark2018arc}, where every decision is refitted with
ARC training and validation questions added to its pool; the free-text
comparison adds Mistral-7B-Instruct-v0.3 as a third family. On MMLU~\citep{hendrycks2021mmlu} both models are prompted to reason
...
where the confidence direction separates confident from hesitant answers best
or within $0.016$ AUROC of the best layer on the tuning split
...
baselines are prompting, the global shift and CAST. All methods are reimplemented on our hooks
...
pass. Its action is a transport map of Theorem~\ref{thm:ot} on the behavioral
coordinate: shifts at several doses and ablation, and on free text also the
quantile map onto the projection law of clean continuations. One rule selects
...
\begin{axis}[name=ceil, at={(law.east)}, anchor=west, xshift=1.3cm, width=.35\linewidth, height=4.4cm,
  title={(b) the decision raises the ceiling}, title style={font=\footnotesize},
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{The decomposition at work.} (a) Selectivity of 480 gated
configurations on the tuning split against the value
Proposition~\ref{prop:account} predicts from ungated runs. (b) Ceiling
$\max(\mathrm{TPR}-\mathrm{FPR})$ of a threshold on each statistic, computed
from unsteered answers, next to the held-out selectivity of PTS. (c) Held-out
AUROC of the PTS probe when it reads only the prompt or the first tokens of the
answer.}
\label{fig:law}\label{fig:decision}
\end{figure}

A global shift of the confidence coordinate removes overconfident-wrong
answers only together with confident-right ones: at $-0.75\lVert h\rVert$ it
removes every answer in both states (Fig.~\ref{fig:frontier}a).
Corollary~\ref{cor:additive}b accounts for this. On the held-out questions the
two confident states overlap on the coordinate the edit moves, and the best
threshold there separates them by $\max(\mathrm{TPR}-\mathrm{FPR})=0.216$ on
Gemma and $0.072$ on Qwen. The detection statistic of PTS, a probe fitted on a
separate pool of 4{,}000 labeled answers, raises this ceiling to $0.370$ and
$0.449$ (AUROC $0.741$ and $0.785$; Fig.~\ref{fig:law}b). Both numbers come
from unsteered answers, before any steered rollout.
...
over 240 configurations on Gemma and $0.91$ over 240 on Qwen. A post-hoc gated
arm returns, question by question, either the unsteered or the ungated output,
so twelve real gated arms on the three held-out splits lie within $0.025$ of
their composition from one ungated run, with a median gap of $0.005$
(Appendix~\ref{app:law}). Tuning a gated operator therefore costs one
generation per action.
...
questions.} Gemma-2-2B, 3{,}000 MMLU questions (unsteered accuracy $0.559$,
ECE $0.354$; $n_{\ocw}{=}963$, $n_{\crok}{=}1{,}399$). $\Delta$Sel is the row
minus PTS post-hoc, with Holm-adjusted $p$ over the seven baselines. Middle
block: PTS decided before the answer. Last two rows: the PTS decision with no
edit, and with a random direction at the PTS dose (mean of 100). $^*$: $95\%$
interval excludes $0$; $^\dagger$: accuracy interval reaches $-0.02$.}
\label{tab:confirm}
\end{table}

Table~\ref{tab:confirm} compares every method at the configuration it won on
the tuning split. Post-hoc PTS reaches selectivity $0.294$ $[0.256, 0.334]$,
removing $49\%$ of overconfident-wrong answers while keeping $80\%$ of
...
differences survive Holm adjustment, and the lead over the CAST-style
condition holds at confidence cutoffs of $0.4$ ($+0.044$ $[0.007, 0.080]$) and
$0.6$ ($+0.068$ $[0.028, 0.107]$). Over all questions, PTS lowers the share of
...
$[-0.009, 0.015]$. Deciding from the first 32 generated tokens, in a single
pass, gives the same selectivity, $0.294$.

The confidence direction carries the edit. Keeping the PTS decision and dose
and replacing the direction by random unit directions gives a mean selectivity
of $0.131$ over one hundred draws; PTS is above all of them and above their
mean by $0.163$ $[0.140, 0.188]$ (gray points in Fig.~\ref{fig:frontier}a), and
on ARC it lies above their 95th percentile. The operator also stays inside the
behavior: gated by the prompt decision, the shift fires on none of the
HumanEval problems and leaves pass@1 at its unsteered $0.396$, where the same
shift applied to every input lowers it to $0.281$
(Appendix~\ref{app:capability}).

\section{PTS across models, behaviors and methods}
\label{sec:transfer}
...
\hline
\end{tabular}}
\caption{\textbf{A second model family and a second benchmark.} Notation as in
Table~\ref{tab:confirm}; unsteered accuracy $0.737$ (Qwen) and $0.744$ (ARC).
Qwen refits every component; ARC refits the decisions and configurations. The
random-direction row averages 100 directions on ARC and 30 directions displaced
as far as the ablation selected on Qwen.}
\label{tab:confirm2}
\end{table}

\subsection{A second model family and a second benchmark}

On Qwen2.5-7B the tuning rule selects ablation as the action, and PTS reaches
selectivity $0.172$ $[0.136, 0.210]$, $0.081$ above regenerating the flagged
questions without an edit and significantly above the CAST-style trace
condition (Table~\ref{tab:confirm2}). On ARC-Challenge, with every decision
refitted, PTS reaches $0.235$ $[0.168, 0.303]$, lowers the calibration error by
$0.060$, and is significantly above Linear-AcT and the CAST prompt condition.
...
\hline
\end{tabular}}
\caption{\textbf{PTS on free text.} 3{,}000 held-out RealToxicityPrompts prompts
per model. rem./kept: percent of toxic continuations removed and of clean
continuations kept (clean and within embedding cosine $0.5$ of the unsteered
text). $\Delta$Sel is the row minus PTS with the shift. The transport map is
the tuning-rule choice on Gemma; the last two rows keep the PTS decision.}
\label{tab:toxicity}
\end{table}

A multiple-choice answer carries one confidence readout; free text carries
none that a decision could simply overwrite, which makes it the natural test of
the action itself. We steer toxic continuation of
RealToxicityPrompts~\citep{gehman2020realtoxicityprompts}, the setting in which
Linear-AcT reports toxicity mitigation, in three model families. A RoBERTa
classifier labels a continuation toxic at score $0.5$, and a clean continuation
counts as kept only if the steered one is clean and within sentence-embedding
cosine $0.5$ of the unsteered one, so rewriting a clean text counts as damage.
The direction is the difference of mean activations between toxic and clean
continuations, and the decision is a probe on the prompt, read before
generation.

On 3{,}000 held-out prompts per model (Table~\ref{tab:toxicity}), PTS is more
selective than the global shift and prompting on Gemma and Qwen, and in all
three families more selective than refusing on its own decision, which gives up
every clean continuation the decision flags. On Gemma the tuning rule selects the quantile
transport map of Theorem~\ref{thm:ot}, from the law of the projection over
toxic continuations to its law over clean ones. It reaches selectivity $0.584$
...
$[0.021, 0.117]$. On Mistral-7B, where toxic continuations are rare, PTS lowers
the toxic rate from $3.4\%$ to $1.4\%$ and has the highest selectivity of the
methods that do not regenerate, $0.520$ $[0.430, 0.605]$. In every family PTS
lies above the 95th percentile of a hundred random directions sent through its
decision, and for each of the seven gated arms we compared with refusal,
Eq.~\eqref{eq:override} evaluated with tuning-split conversion rates predicts
the held-out winner.

\subsection{The setting of PCHI}

PCHI~\citep{liu2026probe} gates a learned rescaling of 128 attention heads with
a probe read where the model states its confidence. We rebuild its setting:
Qwen3-4B-Instruct answers OpenMathInstruct-2 problems in a JSON format that
ends with a yes/no confidence field, and a probe on the layer-18 state at that
field flags overconfident-wrong answers. PTS keeps the probe and replaces the
learned rescaling by a shift of the same state along one direction. On 3{,}183
held-out answers PTS reaches selectivity $0.438$ against $0.354$ for PCHI
($+0.084$ $[0.058, 0.110]$) and a calibration error of $0.188$ against $0.234$
(Appendix~\ref{app:pchi}).

\subsection{Cost}

Deciding from the prompt costs one pass over the prompt and one dot product.
On one H100, 750 toxicity continuations with Gemma take $5.79$ seconds
unsteered and $6.31$ seconds with PTS; a post-hoc decision regenerates the
flagged half and takes $8.74$ seconds (Appendix~\ref{app:latency}). Per token
the action adds one dot product, one 41-knot lookup and one vector update.

\section{Limitations}

Our study has limitations. When a behavior is a single readout, as
multiple-choice confidence is, overwriting that readout on the answers the PTS
decision flags matches the selectivity of the action, which adds most on free
text. The evidence covers two behaviors in three model families of 2 to 7
billion parameters with one toxicity classifier, and the multiple-choice
protocols became public together with their results.
...
ungated run and bounds a decision from unsteered answers alone.

On held-out questions PTS was the most selective of eight methods, at $0.294$
against $0.248$ for the strongest baseline, with a fifteen-point drop in
overconfident-wrong answers. On free text it outperformed the global shift,
prompting and refusal on its own decision, and its quantile transport map
outperformed a shift of equal displacement. We expect the same accounting to
guide conditional control of other behaviors whose target and protected states
