...
measured selectivity without running the gated edit, and it predicts when a
decision is worth adding: when the action alone changes the protected state
often. On answer-level overconfidence in multiple-choice reasoning, in a
pre-registered comparison with baselines tuned under the same budget, PTS is
the most selective method on Gemma-2-2B and is statistically level with the
best methods on Qwen2.5-7B, at unchanged accuracy on both. On Gemma the
decision can be taken from the prompt or a short prefix without losing
selectivity, so the edit runs in a single pass.
\end{abstract}
...
\texttt{Qwen2.5-7B-Instruct}. On Gemma, PTS is more selective than prompting,
additive steering, directional ablation, MiMiC, Linear-AcT and two CAST
variants; on Qwen, where the selected action already spares the protected
state, it is level with the best of them, as the identity predicts. Accuracy
is non-inferior at a two-point margin on both (Sec.~\ref{ssec:confirm}).
\item A single-pass operator: the decision reaches most of its full-trace
detection quality from the prompt or from the first tokens of the answer, and
on Gemma a decision taken there matches the post-hoc one, so PTS runs in one
generation at the cost of two dot products per token
...
(Appendix~\ref{app:capability}). Refitting the probes with out-of-domain
instructions as negatives stops them from firing on coding and open-ended
prompts altogether, and leaves the confirmatory selectivity within $0.015$.
