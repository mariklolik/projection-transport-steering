...
steering can beat it. On answer-level overconfidence, in a pre-registered
comparison across two model families, PTS is the most selective steering method
on one model and level with the best on the other. The account explains both
results: the decision determines the outcome, and detect-and-override with the
same decision matches the best gated edit on one model and far exceeds it on
the other.
\end{abstract}
...
benchmark. PTS is the most selective method on Gemma, significantly so against
seven baselines, and level with the best methods on Qwen
...
\begin{table}[t]\scriptsize\setlength{\tabcolsep}{2.2pt}
\centering
\begin{tabular}{l|cccc|cccc}
\hline
 & \multicolumn{4}{c|}{Gemma-2-2B ($n_{\ocw}=963$, $n_{\crok}=1{,}399$)} & \multicolumn{4}{c}{Qwen2.5-7B ($n_{\ocw}=563$, $n_{\crok}=2{,}007$)} \\
method & $\Delta$acc & ret. & Sel $\uparrow$ & $\Delta$Sel ($p$) & $\Delta$acc & ret. & Sel $\uparrow$ & $\Delta$Sel ($p$) \\
\hline
\inputtab{gen/confirm_side}
\hline
\end{tabular}
\caption{\textbf{PTS is the most selective method on Gemma and level with the
best methods on Qwen, at unchanged accuracy.} Confirmatory split of 3{,}000 MMLU
questions per model, each method at the configuration it won on the tuning
split under one rule. Retention is over the questions that started
confident-right. Sel intervals are question-level bootstrap intervals ($10^4$
resamples); $\Delta$Sel is paired against PTS post-hoc, with Holm-adjusted $p$
over the seven baselines. $^\dagger$: the $95\%$ interval of $\Delta$acc
reaches below $-0.02$. Rows below the line are secondary.}
\label{tab:confirm}
\end{table}
...
\begin{axis}[width=.62\linewidth, height=4.6cm, xlabel={held-out AUROC of the decision}, ylabel={selectivity},
  xmin=0.49, xmax=0.8, ymin=0.0, ymax=0.42, grid=major, grid style={dashed,gray!20},
  tick label style={font=\scriptsize}, every axis label/.append style={font=\footnotesize},
...
probe reaches $0.433$ $[0.393, 0.472]$. The override also calibrates better:
on Qwen it lowers ECE from $0.216$ to $0.139$ and Brier score from $0.230$ to
