...
Projection-Transport Steering (PTS), a framework that separates when to
intervene from how. Its action is the optimal-transport map of a
...
untouched. Its decision is a likelihood-ratio test on a learned detection
...
predicts gated selectivity and unsteered answers give the ceiling of a
decision. On overconfident multiple-choice reasoning with Gemma-2-2B, PTS is
the most selective of eight methods on held-out questions and lowers
calibration error at non-inferior accuracy. On free-text toxicity in three
model families, PTS is more selective than refusal and random directions on its
own decision and, in two families, than a global shift and prompting; where the
tuning rule selects an edit along the displacement field of the quantile
transport map, it outperforms a shift of equal mean displacement.
\end{abstract}
...
The difficulty is sharpest for a behavior that a model shows only on some
...
right. On Gemma-2-2B, the global shift that eliminates every overconfident-wrong
answer also eliminates every confident-right one (Figure~\ref{fig:frontier}a),
because a shift moves the bulk of the distribution as far as it moves the tail.
...
from the protected one, and the action runs only where the test fires.
Table~\ref{tab:family} places published operators in this frame.
...
selectivity, with the optimal decision and its ceiling (Sec.~\ref{sec:method});
the decomposition predicts measured gated selectivity with correlation $0.97$
(Sec.~\ref{ssec:law}).
\item Held-out evidence on overconfident reasoning, with every configuration
committed before the held-out run: PTS is the most selective of eight methods
on 3{,}000 MMLU questions for Gemma, lowers the share of overconfident-wrong answers by
15 points, and its direction outperforms a hundred random directions sent
through the same decision (Sec.~\ref{sec:results}).
\item Free-text control of toxic continuation in three model families, where
PTS is more selective than refusal on its own decision in each family and the
edit along the transport displacement chosen by the tuning rule outperforms a
shift of equal mean displacement; and a head-to-head in the setting of PCHI, where PTS is more
selective than the learned head rescaling under the same probe
...
same retention. Each marker is one method at its tuned configuration on
Gemma, on held-out questions (a) and prompts (b); up and to the right is
better, and dotted lines mark equal selectivity. The orange curve is one shift
at increasing dose, gray points send random directions through the PTS
decision, and the star is the main PTS arm (post-hoc shift in a, scaled
transport displacement in b).}
\label{fig:frontier}
\end{figure}
...
effect varies widely across inputs. All of these operators apply one edit to
every input, with a shape fixed by hand; Theorem~\ref{thm:ot} derives the shape
from a target law.

\paragraph{Transport and distribution matching.} MiMiC~\citep{singh2024mimic}
...
$k$-dimensional projection and leaves its orthogonal complement fixed; for any
target law, including one with atoms, the cost-minimal edit is then optimal
transport in $k$ dimensions, in closed form for $k=1$.

\paragraph{Deciding when to act.} CAST~\citep{lee2024cast} compares the prompt's
...
pool of our probe. DSAS~\citep{ferrando2026dsas} decouples when to steer from
...
PTS in its setting (Sec.~\ref{sec:transfer}). \citet{zhao2026wired} trace
inflated verbalized confidence to components that write at late token
positions. Activations predict whether an
...
direction separates toxic from clean continuations. We write
...
untouched, and moves $h$ as little as possible, since large edits damage
generation (Sec.~\ref{sec:results}).
...
atomlessness of $\mu_V$ then suffices and $\nu$ may have atoms. Directional
ablation is the map onto $\delta_0$, a clamp at $c$ the map onto the law of
$\min(q,c)$, and the shift $S(q)=q+\alpha$ the map onto a translate of
$\mu_V$; for Gaussian marginals $S$ is
...
made on a tuning split (Sec.~\ref{sec:exp}). For the shift, the theorem gives
...
\paragraph{The decision and its accounting.} A decision is needed because the
two states we must tell apart look alike to the action. Overconfident-wrong
(\ocw{}) and confident-right (\crok{}) answers are both confident, and their
laws on the confidence coordinate nearly coincide (Sec.~\ref{sec:results}), so
Corollary~\ref{cor:additive}(b) suggests that a transfer function of that
coordinate changes both at similar rates, as the global shift largely does
(Fig.~\ref{fig:frontier}). A decision therefore reads a second statistic $s$ of
the input and runs the action on an event $\Gamma$ determined by $s$, in
...
flagged answers. Since a gated operator with a post-hoc or prompt decision
returns, input by input, the unsteered or the ungated output, one ungated run
per action composes every such configuration, and the ungated rates predict its
selectivity when the action converts flagged and unflagged answers of a state
alike (Sec.~\ref{ssec:law}). The decomposition also fixes three reference arms
on the same decision: a no-edit arm measures what regeneration alone changes,
a random direction at the same dose what any perturbation of that size
changes, and an override of the reported confidence of every flagged answer
($\rho_O=\rho_C=1$; on free text, a refusal) has selectivity
$\mathrm{TPR}-\mathrm{FPR}$. The action beats that override exactly when
\begin{equation}
...
converts both states alike, and Appendix~\ref{app:proofs} treats the general
case in which conversion depends on $s$. In practice the decision is a
logistic-regression probe, an estimate of the log-likelihood ratio up to a
...
between $0$ and $1$ with positive probability (Prop.~\ref{prop:dose}). A
post-hoc decision scores the finished answer and regenerates the flagged
inputs, so it generates them twice; Proposition~\ref{prop:seq} gives the
...
\paragraph{Models and behavior.} The main model is
Gemma-2-2B-it~\citep{gemma2024} and the main behavior is overconfidence on
MMLU~\citep{hendrycks2021mmlu}. The multiple-choice comparison is repeated on
Qwen2.5-7B-Instruct, a second model family on which every component is
refitted, and on ARC-Challenge~\citep{clark2018arc}, where every decision is
refitted with ARC training and validation questions added to its pool; the
free-text comparison adds Mistral-7B-Instruct-v0.3 as a third family. On
multiple-choice questions both models reason inside
\texttt{<think>...</think>} and answer \verb|\boxed{letter}|, with greedy
decoding. The action runs at layer 14 of 26 (Gemma) and 14 of 28 (Qwen), where
the confidence direction separates confident from hesitant answers best or
within $0.016$ AUROC of the best layer on the tuning split
(Appendix~\ref{app:layers}). Section~\ref{sec:transfer} adds toxic continuation
as a free-text behavior and the setting of PCHI~\citep{liu2026probe}, the
closest conditional method.

\paragraph{Readout.} The primary readout (M5) scores the answer the model
writes. Correctness compares the boxed letter with the gold answer. Confidence
is $P(\mathrm{YES})$ when the model is then asked, in the same conversation,
...
confident-right (\crok{}, right). Appendix~\ref{app:secondary} reports two
secondary readouts, the letter logit of the same answer and a per-option
verification, and Appendix~\ref{app:prompts} gives the prompts.

\paragraph{Splits.} The MMLU splits are disjoint subsets of the test set: 400
...
\paragraph{Metrics.} Each question is paired with itself before and after the
...
\paragraph{Methods and tuning.} We compare PTS with seven baselines: prompting
with a calibration instruction, additive CAA~\citep{rimsky2024caa}, directional
...
reimplemented on our hooks and scored with the same readout. PTS decides
post-hoc, from the finished unsteered answer, and regenerates the flagged
questions; at the prompt, in one forward pass over it before generation; or at
a prefix of $t\in\{16,32,64,128\}$ generated tokens, which keeps decision and
edit in one pass. Its action is a transport map of Theorem~\ref{thm:ot} on the
projection: shifts at several doses and ablation, and on free text also the
displacement field of the quantile map onto the projection law of clean
continuations, scaled by $\lambda\in\{1,2,3,4,6\}$. One rule
selects every configuration on the tuning split: the highest selectivity among
...
\paragraph{Reference arms and statistics.} Every conditional arm is run against
three references that share its decision: a no-edit arm, which regenerates
the flagged inputs with the identity action; random unit directions at the same
dose (ten per conditional multiple-choice arm; one hundred for PTS, or thirty
displaced as far as the selected ablation on Qwen-MMLU); and an override that
marks every flagged answer as not confident, or replaces every flagged
continuation by a refusal on free text. Intervals are $95\%$ paired bootstrap
intervals over questions (prompts on free text) with $10^4$ resamples. The
seven comparisons of PTS with the baselines are two-sided and Holm-adjusted
within each setting, each later protocol Holm-adjusts its own tests, and an
accuracy change is non-inferior when the lower end of its interval is above
$-0.02$. Every protocol was committed before its held-out rollouts, the
multiple-choice one with the hashes of its selected configurations and fitted
detectors (Appendix~\ref{app:protocol}).
...
\begin{axis}[at={(ceil.east)}, anchor=west, xshift=1.35cm, width=.35\linewidth, height=4.4cm,
  title={(c) the prompt already decides}, title style={font=\footnotesize},
...
PTS probe raises the ceiling of a decision. (a) Gated selectivity of 480
tuning-split configurations against its prediction from ungated runs, with
equality dashed; (b) ceiling $\max(\mathrm{TPR}-\mathrm{FPR})$ of a threshold
on each statistic, from unsteered answers, next to held-out PTS selectivity;
(c) held-out AUROC of the PTS probe when it reads only the prompt or the first
answer tokens.}
\label{fig:law}\label{fig:decision}
\end{figure}

A learned detection statistic lifts the selectivity ceiling of a threshold
decision above that of the confidence coordinate on both models. A global
shift of that coordinate removes overconfident-wrong answers only together with
confident-right ones (Fig.~\ref{fig:frontier}a): on the held-out questions the two
confident states overlap on the coordinate, and the best threshold there
separates them by $\max(\mathrm{TPR}-\mathrm{FPR})=0.216$ on Gemma and $0.072$
on Qwen. The PTS probe, fitted on a separate pool of 4{,}000 labeled answers,
raises this ceiling to $0.370$ and $0.449$ (AUROC $0.741$ and $0.785$;
Fig.~\ref{fig:law}b). Both ceilings are computed from unsteered answers, before
any steered rollout.
...
gated selectivity composed from the same run (Fig.~\ref{fig:law}a): the correlation is $0.97$
over 240 configurations on Gemma and $0.91$ over 240 on Qwen. Because a gated
arm with a post-hoc or prompt decision returns, question by question, either
the unsteered or the ungated output, its selectivity can also be composed
from one ungated run; on the three held-out splits, twelve real gated
arms lie within $0.025$ of this composition, with a median gap of $0.005$
(Appendix~\ref{app:law}), so tuning a gated operator costs one generation per
action.
...
Gemma, 3{,}000 MMLU questions (unsteered accuracy $0.559$, ECE $0.354$;
$n_{\ocw}{=}963$, $n_{\crok}{=}1{,}399$); $\Delta$Sel is the row minus PTS
post-hoc, with Holm-adjusted $p$ over the seven baselines. The middle block
holds the PTS decisions taken before the answer; the last three rows keep the
PTS decision with no edit, with a random direction at the PTS dose (mean of
100), and with the reported confidence overridden (Eq.~\eqref{eq:override}). $^*$: $95\%$
interval excludes $0$; $^\dagger$: accuracy interval reaches $-0.02$.}
\label{tab:confirm}
\end{table}

With every method at the configuration it won on the tuning split, post-hoc
PTS is the most selective of the eight on the held-out questions
(Table~\ref{tab:confirm}). It reaches selectivity $0.294$ $[0.256, 0.334]$: it removes $49\%$
of overconfident-wrong answers and keeps $80\%$ of confident-right ones. The
closest baseline, the CAST-style trace condition, reaches $0.248$, and the
other six trail PTS by $0.074$ to $0.233$; all seven differences survive Holm
adjustment, and the lead over the CAST-style condition holds at confidence
cutoffs of $0.4$ ($+0.044$ $[0.007, 0.080]$) and $0.6$ ($+0.068$
$[0.028, 0.107]$). Over all questions, PTS lowers the share of
overconfident-wrong answers by $15.0$ points $[13.7, 16.4]$ and the calibration
error from $0.354$ to $0.255$, with an accuracy change of $+0.003$
$[-0.009, 0.015]$. A decision read from the first 32 generated tokens, in a
single pass, gives the same selectivity, $0.294$.

The confidence direction carries the edit. With the PTS decision and dose
kept and the direction replaced by random unit directions, selectivity averages
$0.131$ over one hundred draws; PTS is above every draw and above their mean by
$0.163$ $[0.140, 0.188]$ (gray points in Fig.~\ref{fig:frontier}a). On code
synthesis the prompt decision keeps the edit off: it fires on none of the
HumanEval problems, so pass@1 stays at its unsteered $0.396$, whereas the same
shift applied to every input lowers it to $0.281$
...
\hline
\end{tabular}}
\caption{On a second model family and a second benchmark, PTS stays above its
no-edit reference arm. Notation as in Table~\ref{tab:confirm}; unsteered
accuracy $0.737$ (Qwen) and $0.744$ (ARC). The random-direction row averages
100 directions on ARC and, on Qwen, 30 directions displaced as far as the
selected ablation; the last row overrides the reported confidence on the
PTS decision.}
\label{tab:confirm2}
\end{table}

\subsection{Further multiple-choice settings}

PTS remains selective on a second model family and on a second benchmark
(Table~\ref{tab:confirm2}). On Qwen
the tuning rule selects ablation as the action, and PTS reaches selectivity
$0.172$ $[0.136, 0.210]$, $0.081$ above regenerating the flagged questions
without an edit and significantly above the CAST-style trace condition. On
ARC-Challenge, with every decision refitted, PTS reaches $0.235$
$[0.168, 0.303]$, lowers the calibration error by $0.060$, is significantly
above Linear-AcT and the CAST prompt condition, and lies above the 95th
percentile of a hundred random directions sent through its decision.
...
\hline
\end{tabular}}
\caption{PTS has the highest single-pass selectivity on free text in all three
families. 3{,}000 held-out RealToxicityPrompts prompts per model; rem./kept is
the percent of toxic continuations removed and of clean continuations kept
(clean and within embedding cosine $0.5$ of the unsteered text), and $\Delta$Sel
is the row minus PTS with the shift. Post-hoc PTS regenerates the prompts it
flags, the scaled transport displacement is the tuning-rule choice on Gemma, and the last two
rows keep the PTS decision.}
\label{tab:toxicity}
\end{table}

Free text is the natural test of the action, because a continuation carries
no single readout that a decision could overwrite in its place. We steer toxic continuation of
...
at score $0.5$, and a clean continuation counts as kept only if the steered one
is clean and within sentence-embedding cosine $0.5$ of the unsteered one, so
...
On 3{,}000 held-out prompts per model (Table~\ref{tab:toxicity}), PTS is more
selective in every family than refusing on its own decision, which gives up
every clean continuation the decision flags, and lies above the 95th
percentile of a hundred random directions sent through that decision; on Gemma
and Qwen it is also more selective than the global shift and prompting. On
Gemma the tuning rule selects $h+3V^\top(S(Vh)-Vh)$, three times the
displacement of the quantile map $S$ from the law of the projection over toxic
continuations to its law over clean ones. This edit reaches selectivity $0.584$
$[0.542, 0.624]$, lowers the toxic rate from $15.0\%$ to $3.5\%$ while keeping
$79\%$ of clean continuations, and is above a shift of the same mean
displacement by $0.043$ $[0.012, 0.076]$ and above CAST by $0.069$
$[0.021, 0.117]$. On Qwen, PTS reaches $0.534$ $[0.496, 0.570]$, also
significantly above CAST, and on Mistral, where toxic continuations are rare,
it lowers the toxic rate from $3.4\%$ to $1.4\%$ at selectivity $0.520$
$[0.430, 0.605]$. For each of the seven gated arms we compared with refusal,
Eq.~\eqref{eq:override} evaluated with tuning-split conversion rates predicts
the held-out winner. On a second, previously unused set of 3{,}000 prompts per
model, under a protocol pushed before its runs, PTS again exceeds refusal on
its own decision by $0.15$ to $0.17$ and a hundred random directions by $0.23$
to $0.43$ in all three families, and prompting in all three
(Appendix~\ref{app:fresh}).

\subsection{The setting of PCHI}

In the setting of PCHI~\citep{liu2026probe}, a shift along one direction
under the same probe is more selective than the learned head rescaling. PCHI
gates a rescaling of 128 attention heads with a probe read where the model
states its confidence. We rebuild its setting: Qwen3-4B-Instruct answers OpenMathInstruct-2
problems in a JSON format that ends with a yes/no confidence field, and a
probe on the layer-18 state at that field flags overconfident-wrong answers.
PTS keeps the probe and replaces the learned rescaling by a shift of the same
state along one direction. On 3{,}183 held-out answers PTS reaches selectivity
$0.438$ against $0.354$ for PCHI ($+0.084$ $[0.058, 0.110]$) and a calibration
error of $0.188$ against $0.234$ (Appendix~\ref{app:pchi}).

\subsection{Inference cost}

With a prompt decision, PTS adds $9\%$ to generation time. On one H100, 750
toxicity continuations with Gemma take $5.79$ seconds unsteered and $6.31$
seconds with PTS, of which $0.46$ seconds go to the pass over the prompts that
reads the decision; a post-hoc decision regenerates the flagged half and takes
$8.74$ seconds (Appendix~\ref{app:latency}).

\section{Limitations}

Where a behavior has a single readout, as multiple-choice confidence does,
overriding that readout on every answer the post-hoc PTS decision flags is at
least as selective as the action (Appendix~\ref{app:refs}). On free text, a
per-token gate without the input decision matches PTS on a fresh split
(Appendix~\ref{app:fresh}). The evidence covers two behaviors in three model
families of 2 to 7 billion parameters with one toxicity classifier, and the
multiple-choice protocols became public together with their results.

\section{Conclusion}

We have introduced Projection-Transport Steering, which splits an activation
edit into an action, a transport map on the projection onto a behavioral
subspace, and a decision, a likelihood-ratio test that sets where the action
runs. Once the subspace and the target law are fixed, the cost-minimal action
is the optimal-transport map between the projection laws, and the selectivity
of the composed operator equals the decision's error rates weighted by the
action's conversion rates. This identity predicts gated configurations from one
ungated run and bounds a decision from unsteered answers alone. In the
main multiple-choice setting PTS was the most selective of eight methods, and
on free-text toxicity it was more selective than refusal on its own decision in
all three model families. We expect the same
accounting to guide conditional control of other behaviors whose target and
protected states share a representation.
