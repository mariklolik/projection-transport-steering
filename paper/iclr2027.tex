...
optimal-transport map from the projection on a low-dimensional behavioral
...
predicts the selectivity of every gated configuration. On overconfident
...
than a global shift, prompting, and refusal on the same decision.
\end{abstract}
...
(Figure~\ref{fig:frontier}), at a cost of fifteen accuracy points. A smaller coefficient
...
this identity predicts the selectivity of every gated configuration. When the
...
flags. The same test can read the prompt or a short prefix, so PTS decides once
per input and edits in the same pass, at two dot products and one table lookup
...
it is level with or above every baseline after Holm adjustment
...
confident-right ones. Grey points keep the PTS decision and dose and replace
...
target law, and the additive edits and ablation are its members for a
translated target and for a point mass at zero.
...
reduces the cost-minimal edit, for any target law and atoms included, to
...
PTS in its setting (Appendix~\ref{app:pchi}). Activations predict whether an
...
reference arms it implies, which we run for every conditional method, and it
decides before its action starts.
...
ungated run per action composes every gated configuration, and the ungated
rates predict it when the action converts flagged and unflagged answers of a
state alike (Sec.~\ref{ssec:law}). The identity also fixes three reference
arms on the same decision: an identity action measures what regeneration alone
...
regenerating the flagged ones doubles the cost of serving; deciding while the
answer is written does not.
...
repeated on \texttt{Qwen2.5-7B-Instruct}, a second model family with its own
directions, probes and tuning, and on ARC-Challenge~\citep{clark2018arc}, where
...
Section~\ref{sec:transfer} adds a free-text behavior, toxic continuation, and
the setting of the closest conditional method, PCHI~\citep{liu2026probe}.
...
turns. Each question falls into one of four states: overconfident-wrong
(\ocw{}, wrong with $P(\mathrm{YES})\ge0.5$), nonconfident-wrong,
nonconfident-right, and confident-right (\crok{}, right with
$P(\mathrm{YES})\ge0.5$). Appendix~\ref{app:secondary} reports two secondary
...
a CAST-style shift conditioned on the finished trace, which, like post-hoc PTS,
regenerates the questions it flags. All methods are reimplemented on our hooks
and scored with the same readout. PTS decides from the finished unsteered
answer and regenerates the flagged questions (post-hoc), or decides from the
prompt, in one forward pass over it before generation, or from the first
$t\in\{16,32,64,128\}$ generated tokens, which keeps decision and edit in one
pass. Its
action is a transport map of Theorem~\ref{thm:ot} on the confidence coordinate,
...
three references that share its decision: an identity action, which regenerates
the flagged inputs with no edit; random unit directions at the same dose (ten
...
flagged answer as not confident, so that its selectivity is
$\mathrm{TPR}-\mathrm{FPR}$, or replaces every flagged continuation by a
...
non-inferior when the lower end of its interval is above $-0.02$. The protocol
of the multiple-choice comparison, with the hashes of its selected
configurations and fitted detectors, was committed to a local repository before
its confirmatory rollouts and first pushed together with its results; the
protocols of the reference arms, the free-text behavior and the PCHI setting
were pushed to a public repository before any of their held-out rollouts
...
the AUROC of the trace direction used by the CAST-style condition.}
\label{fig:law}\label{fig:decision}
\end{figure}

A global edit strong enough to remove the behavior also removes the answers it
should keep. On the Gemma tuning split the additive edit at $-0.75\lVert
...
By Eq.~\eqref{eq:account} and Theorem~\ref{thm:gate}b, an override, or any
action that converts both states at the same rate, reaches at most
$\max(\mathrm{TPR}-\mathrm{FPR})$ of its decision statistic, a ceiling computed
...
median gap of $0.005$ (Appendix~\ref{app:law}). We therefore score every gated
configuration during tuning from one generation per action, and configurations
that decide before the answer by real runs. With $\rho_O=\rho_C=\rho$, Eq.~\eqref{eq:account}
is $\rho\,(\mathrm{TPR}-\mathrm{FPR})$, which the ceiling above bounds.
...
confident-right ones. The closest baseline, the CAST-style shift conditioned on
the finished trace, reaches $0.248$, and the other six trail PTS by $0.074$ to
$0.233$; all seven differences remain significant after Holm adjustment. The
lead over the CAST-style shift holds when the confidence cutoff moves from
...
\textbf{Code generation.} Gated by the prompt decision, the shift leaves
HumanEval unchanged: the decision fires on none of the problems, so pass@1
stays at its unsteered $0.396$. The same shift applied to every input at
$-0.5\lVert h\rVert$, the dose selected for the prompt decision, lowers pass@1
to $0.281$ (Appendix~\ref{app:capability}).\section{PTS Beyond the Main Setting}
\label{sec:transfer}
...
$[0.136, 0.210]$, $0.081$ above its no-edit null. After Holm adjustment it is
...
reaches $0.235$ $[0.168, 0.303]$ and lowers ECE by $0.060$. It is above Linear-AcT and the CAST prompt condition, whose
decision is at chance on ARC (AUROC $0.51$), and level with the additive edit
and the CAST-style trace condition, which reach $0.294$ and $0.289$ (Holm
$p=0.36$), and two of its hundred random directions score above it.
...
decision.} Free text has no single readout that a decision could overwrite,
...
Since the decision gates each row inside the original batch, its no-edit null
...
field, a probe on the layer-18 state at that field flags wrong-but-confident
...
\textbf{With out-of-domain negatives, the decision stays silent on code and
open instructions.} Refitted with 600 AlpacaEval instructions as negatives, the
probes fire on none of the HumanEval and AlpacaEval prompts, so the operator is
the exact identity there, and they keep held-out selectivity at $0.280$ and
$0.300$ (Appendix~\ref{app:capability}).

\textbf{Deciding from the prompt costs a tenth of the unsteered wall clock.}
...
traces boxed in both arms, the lead over the CAST-style shift is $+0.032$
...
families of up to seven billion parameters. Fifth, the configuration hashes of
...
projection laws, and the additive edit is that map only when the target is a
...
In a comparison frozen before its held-out run, PTS is the most selective of
...
and the calibration error. On free text, deciding from the prompt before generating, it is above global steering and above
refusing on its own decision in two model families.
...
multiple of the mean activation norm. The single-pass decision is a logistic
