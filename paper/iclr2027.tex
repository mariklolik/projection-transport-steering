...
tuning rule selects the quantile transport map, it outperforms a shift of
equal displacement.
\end{abstract}
...
quantile transport map chosen by the tuning rule outperforms a shift of equal
mean displacement; and a head-to-head in the setting of PCHI, where PTS is more
...
decision, and the star is the main PTS arm (post-hoc shift in a, quantile
transport map in b).}
\label{fig:frontier}
\end{figure}
...
by Corollary~\ref{cor:additive}(b) any transfer function of that coordinate
changes both at similar rates, as the global shift does
...
projection: shifts at several doses and ablation, and on free text
also the quantile map onto the projection law of clean continuations. One rule
...
confident-right ones (Fig.~\ref{fig:frontier}a), as
Corollary~\ref{cor:additive}(b) predicts: on the held-out questions the two
...
measured gated selectivity (Fig.~\ref{fig:law}a): the correlation is $0.97$
...
flags, the transport map is the tuning-rule choice on Gemma, and the last two
...
Gemma the tuning rule selects the quantile transport map of
Theorem~\ref{thm:ot}, from the law of the projection over toxic continuations
to its law over clean ones. The map reaches selectivity $0.584$
...
selection rule picks the map at $\lambda=3$. Its mean displacement on toxic
...
it, and it reaches the lowest calibration error of all arms ($0.188$). The
