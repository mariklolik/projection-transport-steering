Across the 223/300 gated questions, the intervention does \emph{not} inject
hedging vocabulary --- hedge-marker frequency \emph{drops} ($0.25\to0.13$ per
trace) --- and traces get shorter ($86\to71$ words). What changes is the mode
of reasoning: assertive shortcut arguments are replaced by definitional,
evidence-first ones. Of the 50 overconfident-wrong traces, 29 leave that state
and 12 come back \emph{correct} under M4, while 55 of the 76 confident-right
traces keep their state. Ablating the confidence projection during
...
, M4 state overconfident wrong $\to$ nonconfident right). The baseline reopens
\texttt{<think>} three times and re-states the same elimination in each block, so it runs out
of budget before reaching D; the steered trace weighs all four options once and lands on the
same letter with calibrated confidence.
...
\textbf{after:} \texttt{\textless{}think\textgreater{}The question asks for the accidental discovery of Pavlov. Option B is the best choice because it describes Pavlov's accidental discovery of the dog's salivation in response to the sight and sound of the attendant who brought them food. \textless{}/think\textgreater{} \textbackslash boxed\{B\}} $\to$ \texttt{B}
\end{quote}
