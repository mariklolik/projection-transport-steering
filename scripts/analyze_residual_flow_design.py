...
def load_packet(directory: Path) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    for shard, expected_ids in EXPECTED_SHARDS.items():
        ...
        expected_forwards = 2 + 59 * len(expected_ids)
        if (
            ...
            or receipt["action_serialization_max_abs_error"] != 0.0
        ...
        result.update(
            {
                ...
                "resources": {
                    "backward_products": sum(
                        receipt["backward_products"] for receipt in receipts
                    ),
