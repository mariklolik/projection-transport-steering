# Pairwise LLM-as-judge on the open-ended generations: each steered answer is
# compared with the unsteered one for the same instruction, in both orders, and
# a win counts only when the judge prefers the same side twice (ties otherwise).
# Run: python -m behaviour_specific.overconfidence.judge_openended --outdir v3_openended
...
def win_rate(v_ab: list[str], v_ba: list[str]) -> dict:
    """Order-consistent win rate: a win needs agreement across both presentations."""
