# Does the edit keep activations on the model's own manifold, including on
# domains the operators were never fitted on? For each operator we teacher-force
# the unsteered trace, apply the operator to the layer-L residuals, and measure
# displacement, norm ratio, Mahalanobis distance to the extraction-split
# activation law, nearest-neighbour distance to a bank of natural activations,
# and the KL the edit induces on the next-token distribution.
# Run: python -m behaviour_specific.overconfidence.diag_manifold --benchmark arc
...
def mahalanobis(x: torch.Tensor, mean: torch.Tensor, prec: torch.Tensor) -> torch.Tensor:
    """Per-row sqrt((x-m)^T P (x-m)) for a precision matrix P."""
    z = x - mean
...
def conditional_gaussian(V: torch.Tensor, mean: torch.Tensor, cov: torch.Tensor,
    ...
    """Law of the edited coordinates q=Vh given the untouched complement.

    A full-space Mahalanobis distance is diluted by the d-k coordinates an edit
    never touches; conditioning on them asks the sharper question, whether this
    projection value is jointly plausible with the rest of the state. We
    condition on the `m` leading principal directions of the complement, which
    is both well conditioned and where the dependence actually lives.
    """
    k, d = V.shape
    P = torch.eye(d) - V.T @ V
    C = P @ cov @ P.T
...
def conditional_distance(h: torch.Tensor, V: torch.Tensor, B, m_q, m_p, K, prec_c):
    """Squared conditional Mahalanobis of the edited coordinates; chi^2_k under the null."""
    q, pth = h @ V.T, h @ B.T
    z = q - (m_q + (pth - m_p) @ K.T)
...
def kl_next_token(model, tok, texts: list[str], fn, layer: int, batch: int = 16) -> float:
    """Mean KL(unsteered || steered) of the next-token law, the standard locality check."""
...
def nn_distance(x: torch.Tensor, bank: torch.Tensor, chunk: int = 512) -> torch.Tensor:
    """Distance from each row of x to its nearest neighbour in `bank`."""
    out = []
...
def _selftest():
    ...
    # independent coordinates: conditioning changes nothing, so the distance is
    # the plain squared Mahalanobis of the edited pair
    Vt = torch.eye(6)[:2]
...
if __name__ == "__main__":
    ...
    # a bank of natural layer-L activations from the unsteered traces themselves
    bank, acts = [], []
