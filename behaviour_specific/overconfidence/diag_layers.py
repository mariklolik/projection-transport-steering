...
if __name__ == "__main__":
    ...
    for L in layers:
        X = torch.stack(trace_means[L])
        A = torch.cat(tok_acts[L]).float()
        per_trace = torch.stack([t.float().mean(0) for t in tok_acts[L]])
        v = unit_diff(per_trace[is_conf_tr], per_trace[~is_conf_tr])
        u = unit_diff(per_trace[is_ocw], per_trace[is_cr])
