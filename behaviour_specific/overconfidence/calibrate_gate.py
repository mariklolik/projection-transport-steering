# The sequential gate needs the scale of the deviation between its running mean
# after t positions and the trace statistic it is estimating. Token-level
# variance overstates it badly, because gate contributions inside one trace are
# strongly correlated; we therefore measure the deviation profile directly on
# the extraction traces and fit the sub-Gaussian proxy that reproduces it.
# Run: python -m behaviour_specific.overconfidence.calibrate_gate --gate-layer 16
...
def fit_sigma(dev: dict[int, torch.Tensor], delta: float) -> float:
    """Smallest sigma whose time-uniform boundary covers the (1-delta) deviation quantile."""
    need = []
...
if __name__ == "__main__":
    ...
    # a gate that must decide at token t should be fitted on what it sees at
    # token t, not on the whole-trace mean it is trying to anticipate
