# Single-pass conditional steering: the LDA trace gate evaluated as a running
# mean while the trace is being written, so nothing is regenerated. Same gate
# statistic, same threshold and same action as the two-pass sweep condition —
# only the moment of the decision changes.
# Run: python -m behaviour_specific.overconfidence.steer_v6_online --outdir v3_mmlu_s7
...
def gate_sigma(cov: torch.Tensor, w: torch.Tensor) -> float:
    """Per-token std of the gate contribution, from the extraction-split token cov."""
...
def load_gate(layer: int, tau_q: float, delta: float = 0.05, decide_at: int | None = None):
    """(V, w, b, tau, sigma) of the LDA gate fitted at `layer` on the extraction split.

    With `decide_at`, the head is the one fitted on exactly that prefix budget,
    so the gate is scored by the statistic it will actually have at that token;
    otherwise the head is the whole-trace one and `sigma` is the calibrated
    running-mean deviation scale that the anytime boundary needs.
    """
