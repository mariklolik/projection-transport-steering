# Gemma Scope pretrained SAEs for gemma-2-2b (residual stream).
#
# These SAEs were trained by DeepMind on the BASE model `gemma-2-2b` (pt); we
# apply them to the instruct model `gemma-2-2b-it`. That cross-application is
# standard for Gemma Scope work but is a real caveat (the residual streams are
# close, not identical) — note it in any result.
#
# The SAE for a layer is one params.npz (~300 MB at width 16k). It is downloaded
# ONCE, LOCALLY (this needs internet), into `gemma_scope/` next to this file, and
# then transferred to the offline machine like the model weights. At runtime
# `load(layer)` just reads that local file — no network.
#
# `python -m models_specific.gemma_2_2b_it.gemma_scope` prints resolved paths.
...
REPO = "google/gemma-scope-2b-pt-res"          # residual-stream SAEs, base gemma-2-2b
...
def download_sae(layer: int, width: str = DEFAULT_WIDTH) -> Path:
    """LOCAL ONLY (needs internet): fetch the SAE params.npz into `gemma_scope/`.
...
def neuronpedia_url(layer: int, feature: int, width: str = DEFAULT_WIDTH) -> str:
    ...
    return f"https://www.neuronpedia.org/gemma-2-2b/{layer}-gemmascope-res-{width}/{feature}"
...
if __name__ == "__main__":
    ...
    print(f"layer {L} npz :", sae_path(L), "(present)" if sae_path(L).exists() else "(missing — download locally)")
